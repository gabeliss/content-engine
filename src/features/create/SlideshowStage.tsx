...
export function SlideshowStage({
  ...
  const selectedStageRef = useRef<HTMLDivElement | null>(null);
  ...
  const centerSelectedElement = (
    ...
    const nextLeft = element.offsetLeft + element.offsetWidth / 2 - container.clientWidth / 2;
    container.scrollTo({
      behavior,
  ...
  useEffect(() => {
    centerSelectedElement(thumbScrollRef.current, selectedThumbRef.current, "smooth");
  }, [selectedSlide.slideId, displaySlides]);
  ...
  return (
    <div className="grid min-w-0 gap-3 rounded-[var(--radius-md)] bg-[var(--color-page-quiet)] px-[var(--space-3)] py-[var(--space-3)] sm:px-[var(--space-4)]">
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-[var(--space-2)]">
        ...
        <div className="min-w-0 overflow-hidden py-1" ref={stageViewportRef}>
          <div
            className="flex w-max items-center gap-[var(--space-3)] transition-transform duration-300 ease-out sm:gap-[var(--space-4)]"
        ...
        <div className="mx-auto flex w-max items-center gap-2 px-1">
          {displaySlides.map((slide) => (
            <button
              ...
              ref={slide.slideId === selectedSlide.slideId ? selectedThumbRef : undefined}
