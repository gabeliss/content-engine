...
const POST_BRIDGE_PROVIDER: PublishingProviderName = "post_bridge";
...
function createPostBridgeHttpError(
  operation: string,
  statusCode: number,
  details: string
): ProviderError {
...
function createDryRunId(prefix: string): string {
  return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
}

function encodeUploadData(input: UploadMediaInput): Uint8Array {
